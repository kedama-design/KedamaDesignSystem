...
status: Draft v0.12 — すらすらスタジオの実態調査（既存UIあり・@kedama/design-system 0.1.0を
  ...
  v0.12でPhase A-2の配布を閉じた：パッケージ名を@kedama-design/design-systemへ確定し
  private解除、タグ/手動releaseでGitHub Packagesへpublish、公開版を新規導入して
...
**位置づけ**：`@kedama-design/design-system`（既存リポジトリ `KedamaDesignSystem`）を横断コンポーネント
...
> **2026-08-03 訂正**：当初ここには「**このリポジトリ自体では実装しない**（本書を実装
> セッションに渡して着手する）」と書いていたが、**実態と食い違うため撤回する**。
> Phase A-1 / A-2 の実装はこのリポジトリで行われており、トークン層・Tier 0 の12部品・
> 配布設定はすべてここにある。本書は「渡す仕様書」ではなく、**このリポジトリの
> 設計と決定の記録**である。
...
- パッケージ名確定済み：`@kedama-design/design-system`（当初 `@kedama` としていたが、
  GitHub Packages はスコープと Organization の一致を要求するため改名。2026-08-03 適用）
...
> **これは 2026-07-28 時点のスナップショットである（2026-08-03 注記）。**
...
> 複数プロダクトで共有するUI基盤 `@kedama-design/design-system` を整備し、**プロトタイピング工程
...
@kedama-design/design-system の Tier 0 / Tier 2
...
> 消費側（すらすらスタジオ）は現在 `@kedama/design-system`（旧名）を import しているため置換が必要
...
**ただし「構成・レイアウト・挙動は Ibuki が正」ではない**（2026-08-03 訂正）。
その条項は 2026-07-30 に撤回済みで、現行は「見た目・構造・スペーシング・
エレベーション・角丸 → **shadcn** が正」（§0.6）。したがって:

- **構造と挙動の正** … shadcn（Base UI variant のブロック）
- **Ibuki のプロトタイプとベンチマーク §7.2** … **要件と参照元**。どのスロットが
...
### Storybookのホスティング先（確定：GitHub Pages）

> **2026-08-03 変更**：当初 Vercel を採用としていたが、**GitHub Pages へ変更した**。
> `GITHUB_TOKEN` だけで完結し、外部サービスの認証情報を持ち回らなくて済む。権限も
> リポジトリと同じものが効く。実装は `.github/workflows/storybook.yml`（main への
> push で自動デプロイ）。
>
> Chromatic の判断には影響しない。あれは Visual Regression が目的で、ホスティングとは
> 別の話（下表のとおり、複数プロダクトが消費し始めた後に価値が上がる）。
